...
cdef tuple inCorner(int x1, int y1, int width, int height):
    dw = round(width / 2.75)
...
def startDetect(videoname, vs ,conf : float, thold : float, outputfile : str, fileonly : bool, drawpaths : bool, maxloss : int, logging : bool,calccross : bool, pixelspercm : float,  isQt : bool, bar):
    ...
    cdef int fwidth, fheight, ffps, totalframes
    cdef float scale_percent
    ...
    cdef np.ndarray blob

    cdef list ln, layerOutputs

    cdef np.ndarray output
    cdef list boxes, confidences, classIDs
    cdef int classID
    cdef np.ndarray box
    cdef float confidence
    cdef np.ndarray detection, scores

    #cdef np.ndarray idxs

    cdef list newCoord, IDs

    cdef int iterator

    cdef int x, y, w, h, centerX, centerY

    cdef list distances

    cdef int idIndex, newID

    cdef int idval
    cdef bint duplicateDetected, oldz

    cdef bint ifCorner
    cdef int whichCorner

    cdef int confidenceround

    cdef str text, fpsinfo
    cdef Py_ssize_t textLen
    cdef list color
    ...
    #cdef float programEnd
    while True:
        start = time.time()
        isframe, frame = vs.read()
        ...
        ln = [ln[outlayer[0] - 1] for outlayer in net.getUnconnectedOutLayers()]
        ...
        for output in layerOutputs:
            for detection in output:
                ...
                if confidence > conf:
                    ...
                    (centerX, centerY, width, height) = box.astype(DTYPE3)
        ...
        if len(idxs) > 0:
            for iterator, i in enumerate(idxs.flatten()):
                ...
                if not previousIDs or framenum == 1:
                    ...
                    IDs.append(newID)
                    newCoord.append([x, y])
                    confidenceround = round(confidences[i] * 100)
                ...
                if not IDs:
                    ...
                    text = "ID:{} {} c: {}%".format("?",objectNames[classIDs[i]], confidenceround)
                    textLen = len(text) * 9
                    ...
                    cv2.rectangle(frame, (x,y), (x + w, y + h), color, 2)
                    text = "ID:{} {} c: {}%".format(IDs[iterator],objectNames[classIDs[i]], confidenceround)
                    textLen = len(text) * 9
                    cv2.rectangle(frame, (x,y), (x + textLen, y - 20), color, -1)
    ...
    print("")
    cdef int pathcrossed
    cdef int x1, x2, x3, x4, y1, y2, y3, y4, v1, v2, v3, v4
